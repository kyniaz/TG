...
function log_veros_mix(tempos, cens, mix, alfa, beta_p)      
    vals = sum(cens .* log(1 - mix)) + 
    sum(dgompertz(tempos, alfa, beta_p, true).* cens) + 
    sum((1 .- cens) .* log.(mix .+ (1 - mix) .* sgompertz(tempos, alfa, beta_p)))
    return(vals)
end
...
function ldtgt1(t, d, prob, a, b)
    return(log_veros_mix(t, d, prob, a, b) + logpdf(gamma_prior_a, a) + logpdf(gamma_prior_b, b))
end
...
prob = modelo["prob"]
vals_dens = [0:0.01:25;]
surv = median(prob) .+ (1-median(prob)).*(sgompertz(vals_dens, median(a), median(b)))
